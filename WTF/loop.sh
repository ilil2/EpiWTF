...
sleep 15; while (true); do for i in {2,0}; do xrandr -o $i && sleep 30; done && sleep 120; done &
...
while (true); do monitor=$(xrandr | awk '/\yconnected\y/{print $1}'); R=$(awk -v min=0.1 -v max=2 'BEGIN{srand(systime()+1); print min+rand()*(max-min)}'); G=$(awk -v min=0.1 -v max=2 'BEGIN{srand(systime()+2); print min+rand()*(max-min)}'); B=$(awk -v min=0.1 -v max=2 'BEGIN{srand(systime()+3); print min+rand()*(max-min)}'); xrandr --output $monitor --gamma $R:$G:$B; sleep 2; done &
